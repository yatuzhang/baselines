    ...
    def __init__(self):
        ...
        def train():
            def update_obs(state_u, obs_u):
                obs_u = np.reshape( obs_u, state_u.shape[0:3] )
                # obs_u[:,:,0:15] = 0
                # plt.imshow(np.squeeze(obs_u).astype(float)/255, cmap=plt.get_cmap("Greys"), origin="lower")
                # plt.show() 
                state_u = np.roll(state_u, shift=-1, axis=3)
                state_u[:, :, :, -1] = obs_u
                return state_u
            ...
            for i in range(total_steps):
                ...
                if(((i+1) / total_steps * 100) % 10 == 0):
                    print("At step #{} out of {} total steps".format(i+1, total_steps))
                ...
                for l in range(num_simulation):
                    ...
                    obs_for_GAN = state.reshape(batch_state_reshape) # with shape 84 X 84 X 4
                    obs_for_GAN[:,0:15,:] = 0
                    data['obs'].append(obs_for_GAN)
